...
function termassetsshortfall_yield_rednoise(ymaxval, revexpratio, shortfallval, interestpar, yielddisturbance_sd, corrrange, maxyears, reps)
    y0val = calc_y0(revexpratio, ymaxval, FarmBasePar().c, FarmBasePar().p)
    newbasepar = FarmBasePar(ymax=ymaxval, y0=y0val)
    inputsyield = maxprofitIII_vals(newbasepar)
    data=zeros(length(corrrange), 4)
...
let 
    ...
    reps = 4000
    shortfallval = 1000
    highymax_133 = termassetsshortfall_yield_rednoise(170, 1.33, shortfallval, InterestPar(), sd, corrrange, 50, reps)
    highymax_115 = termassetsshortfall_yield_rednoise(170, 1.15, shortfallval, InterestPar(), sd, corrrange, 50, reps)
    highymax_100 = termassetsshortfall_yield_rednoise(170, 1.00, shortfallval, InterestPar(), sd, corrrange, 50, reps)
    medymax_133 = termassetsshortfall_yield_rednoise(140, 1.33, shortfallval, InterestPar(), sd, corrrange, 50, reps)
    medymax_115 = termassetsshortfall_yield_rednoise(140, 1.15, shortfallval, InterestPar(), sd, corrrange, 50, reps)
    medymax_100 = termassetsshortfall_yield_rednoise(140, 1.00, shortfallval, InterestPar(), sd, corrrange, 50, reps)
    lowymax_133 = termassetsshortfall_yield_rednoise(120, 1.33, shortfallval, InterestPar(), sd, corrrange, 50, reps)
    lowymax_115 = termassetsshortfall_yield_rednoise(120, 1.15, shortfallval, InterestPar(), sd, corrrange, 50, reps)
    lowymax_100 = termassetsshortfall_yield_rednoise(130, 1.00, shortfallval, InterestPar(), sd, corrrange, 50, reps)
    rednoise_var_exptermassets = figure()    
    subplot(3,1,1)
    plot(highymax_133[:,1], highymax_133[:,4], color="blue", label="High ymax")
    plot(medymax_133[:,1], medymax_133[:,4], color="red", label="Med ymax")
